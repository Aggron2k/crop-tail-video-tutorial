class_name SceneDateResource
extends NodeDataResource

@export var node_name: String
@export var scene_fiel_path: String

func _save_date(node: Node2D) -> void:
	super._save_data(node)
	
	node_name = node.name
	scene_fiel_path = node.scene_file_path
...
func _load_data(window: Window) -> void:
	...
	if node_path != null:
		var scene_file_resource: Resource = load(scene_fiel_path)
